...
class LoginViewModel(private var dataRepo: DataRepository?) : ViewModel() {
    ...
    val changePasswordpData = MutableStateFlow<ResponseModel<Response<ChangePasswordResponse>>>(ResponseModel.Idle("Idle State"))
    ...
    suspend fun changePassword(changePasswordModel: ChangePasswordModel) {
        ...
        dataRepo?.changePassword(changePasswordModel)?.collect {
            viewModelScope.launch {
